...
Proof.
  apply nat_ind.
  - simpl. reflexivity.
  - intros n' IHn'. simpl. f_equal. apply IHn'.
Qed.
(** [] *)
...
Proof.
  intros P H1 H2 H3.
  induction b as [| b' | b' t1 IHt1 t2 IHt2].
  - apply H1.
  - apply H2.
  - apply H3. apply IHt1. apply IHt2. 
Qed.
...
Theorem Toy_correct : exists f g,
  forall P : Toy -> Prop,
    ...
    forall t : Toy, P t.
Proof.
  exists con1, con2. apply Toy_ind.
Qed.
...
(*  Just follow the example of mul_0_r''.
    [] *)
...
Definition better_t_tree_ind_type : Prop :=
  forall (X : Type) (P : t_tree X -> Prop),
    P t_leaf ->
    (forall t1 x t2, P t1 -> P t2 -> P (t_branch (t1, x, t2))) ->
    forall t, P t.
...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun X P Pl Pb => fix f (t : t_tree X) :=
    match t with
    | t_leaf => Pl
    | t_branch (t1, x, t2) => Pb t1 x t2 (f t1) (f t2)
    end.
...
Proof.
  ...
  - simpl. reflexivity.
  - intros t1 x t2 IHt1 IHt2. simpl.
